...
**中转服不计入人数（`transit`）。** 代理的 `/status` 里会列出**全部**子服，其中有一台
不是给人玩的：Velocity 的 `limbo`（排队房 / 中转服）。玩家在登录握手、换服的一瞬、
或者想进的服把他拒了的时候，会被代理放在那里。它的人也算进接口报的「全群组人数」，
于是总览会出现自相矛盾的一幕：
...
差额正站在 limbo 里。所以把这类子服在**代理那条**上点名，它们的人从「全群组」里扣掉：
...
kind    = "proxy"
api     = { url = "http://127.0.0.1:8080", token = "<对方给的 token>" }
transit = ["limbo"]            # 名字要跟对方 velocity.toml 里的服名逐字相同
```

- 只对 `kind = "proxy"` 且配了 `api` 的目标有效（别的写法加载期报错）：没有那张
  `/status` 表，就无从知道中转服上有几个人。
- **名字写错不会报错**，只是一个人都扣不掉 —— 表现就是人数一直偏大一点点，在群里
  完全看不出来。三处都会把它打出来：启动日志、`--list-targets`、以及唯一能逐个对号的
  `--api`（那儿会写「已扣掉中转服 limbo 1 人」或「limbo 不在接口的子服列表里」）。
- 已经挂成子服的那台**不能**再写进 `transit`（会加载期报错）：那样它的人被扣出总数，
  却仍在自己的行里列着，数字照样对不上。
- 接口取不到、退回 SLP 时**扣不掉**（SLP 只给一个总数、没有分服明细）。这时人数会
  临时偏大，快照的 error 里会明说这件事 —— 别把它当成「中转服没人」。

**落地服不推进服（`quiet_join`）。** 群组服里总有一台是**落地服**（大厅）：玩家进服先
落在那里，紧接着就换到真正想玩的那台。两个事件通常落在**同一轮**探测里，于是群里连着
推两条：
...
前半截是重复（他并不是来玩大厅的）。在那台**自己**的配置上写一句，就只推后半截：
...
group      = "群组服"
source     = "szu"
quiet_join = true              # 有人进这台不推进服提醒
```

- 只掐「**落在这台的进服**」这一种事件。别的一律不动：
  - **人数照算** —— 总览、定时播报、`@mc` 查询照旧列出大厅里的人（那几处回答的是
    「现在有几个人可以一起玩」，站在大厅里等的人当然算）；
  - **掉线/恢复照推** —— 那是「这台服挂了」，与谁在玩无关；
  - **换到这台照推**（`🔄 X 从「生电」换到「大厅」`）—— 换服行两端都写在行里，本来
    就不会与谁重复，而且那是「人离开生电」在群里唯一的痕迹（退服本来就不推）。
- 与它同轮到达的换服行照旧发出，所以「进服 → 落大厅 → 换走」这套动作在群里**只剩一行**。
- 写在 `kind = "proxy"` 的目标上会**加载期报错**：代理那条不参与进服对账（它的名单是
  全群组口径，见 `serves_names`），写在它身上永远不会生效。落地服是挂在它下面的**子服**，
  写在子服那条上。
- 启动日志与 `--list-targets` 都会标出来（`（落地服，进服不播报）` / 「落地服：有人进这台
  不推进服提醒」）。刻意标出来是因为这个键**配了没生效**时长得和「本来就该安静」一模一样。
...
群里会明确回一句「🤔 本群还没有开通此功能」并在日志里留一条 warning。
...
「🏗️ 本群关联的服务器还没接入，暂时没有可查询的内容」—— 比笼统的「本群还没有开通此功能」
...
- **MC 查询**：在**已开通**的群里 `@机器人 mc` → 回复该群关联的服务器总览（主服排最前）
  ...
  - 一台服一块，块挨着块（不空行）。**跨 `group` 会画一条虚线**（实测形状：模组服那台与
    群组服那一段之间），好让读者一眼看出哪些服是同一个体系的；只有一组时不画。
    分组的依据是各目标自己的 `group`，不是服名 —— 加一组服不用改渲染代码
  ...
  - ⚠️ **必测跨群隔离**：在一个**没写进** `mcs_audiences.toml` 的群里发 `@机器人 mc`，
    应回「🤔 本群还没有开通此功能」，且日志里有一条同等信息的 warning（见 F10）
  - 若某条关联 `targets = []`（配置里那条占位条目就是，群号是假的所以没法在群里测），
    那个群应回「🏗️ 本群关联的服务器还没接入」——**不是**「本群还没有开通此功能」。
...
- **MC 进服提醒**：自己进服，最慢 `MC_WATCH_INTERVAL_SEC` + `MC_JOIN_MIN_INTERVAL_SEC` 秒内推送「🎮 X 加入了…」（默认 10+15，即 25 秒内；两人紧挨着进服会合并成一条）
  ...
  - **合成一条消息**：`MC_JOIN_MIN_INTERVAL_SEC` 窗口内三台服依次进人 → **一条**消息按服分段列出，不是三条。
  - **排版：一段一台服、行内带服名、段间一条虚线**。**没有消息标题**（没有 `🎮 MC 动态`）、**没有 `【服名】` 块头**、不带时间 —— 服名一律写在行里（带「」，与换服行同一套引号），所以一个人进服就是一行 `🎮 X 加入了「Bingo」`，三台服各有人进就是三行、中间两条 `- - -` 虚线。位置按**玩家最后待在哪台**排：进服落在进的那台、换服落在**到达**的那台。
  - **整条消息一个数字都没有**：人数只在**定时播报**与 `@mc` 查询里报（那两处回答的是「现在几个人可以一起玩」，这里回答的是「谁来了」）。所以进服不带「（当前 N 人在线）」、掉线就一句 `⚠️ Bingo 连不上了`（不带「已连续 N 轮探测失败」，那个数只有看日志的人用得上）、恢复就一句 `✅ Bingo 已恢复`。
  - **同组换服**：在 `group` 相同的两台（`主服群` 里的 bingo / 谁是杀手）之间移动 → 一条 `🔄 X 从「<来源服>」换到「<目标服>」`。**一行说完**：两端服名都在行里，不报「当前 N 人在线」（换服时几台服往往都是 1 人）。跨 group 移动（GTNH ↔ bingo）**只报普通加入**，没有 `🔄`——跨组时玩家确实断线重连了，说「从哪来」是猜。
  ...
  - **落地服不推进服**：大厅那台配了 `quiet_join = true` 之后，自己进服再换到别的子服 → 群里**只有** `🔄 X 从「大厅」换到「<服名>」`，不再先来一条「溜进了大厅」。同时确认**没掐过头**：大厅里的人数照旧出现在总览/播报/`@mc` 里，大厅自己掉线照旧推 `⚠️ 大厅 连不上了`，从别台换回大厅照旧推 `🔄 X 从「生电」换到「大厅」`。判据的完整清单见 4.1 的「落地服不推进服」。
...
- **MC 定时播报**：
  ...
  - **某台离线或没人 → 只影响它那一块**（`😵 不可达` / `在线 0`），整条播报照发。今天那种「单目标 0 人 → 整条不发」在多目标下已经不适用。
    （0 人不再补一句「目前无人」——数字已经说完了；总览同理。）
...
| 「🤔 本群还没有开通此功能」 | 该群没被写进任何 `[[audience]].groups` | 见 F10 情况一 |
