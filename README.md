...
- 🌙 **夜间静默**（MC 与 oopz 各一个键）：`MC_QUIET_HOURS` / `OOPZ_QUIET_HOURS`，默认都是 `0-9`，即 00:00–08:59 内**不发自动消息**，半夜不吵人；`09:00` 整那班照常发，支持跨午夜（`23-7`），留空 = 不静默。MC 那边掐「进服/换服提醒 + 定时播报」，**掉线/恢复提醒不受影响**（夜里服务器真挂了得让人知道）；oopz 那边掐「定时播报 + 进频道欢迎」，**`@oopz` 查询不受影响**（那是人主动问的）。两边静默期间都**照样取数**，所以 09:00 之后不会把整晚的人当成「刚进服 / 刚进频道」补报一遍
