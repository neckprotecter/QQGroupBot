...
> **补记（2026-09-26，`quiet_join` 字段）**：上面那句「他接受两条提醒」翻案了 ——
> 一天后用户说「进入大厅不需要播报，只播报进入其他子服」，理由是那两条里**前一条是
> 重复**（他并不是来玩大厅的）。于是加 `quiet_join = true`，写在**大厅那条子服**上
> （不是代理上），推送层把「落在这台的进服」丢掉。
>
> 三条边界都进了渲染层 docstring 与自测：**只掐进服** —— 人数照算（总览/播报/查询走的是
> **快照**那条路，与进服事件无关）、掉线/恢复照推、换到这台照推。最后一条是刻意的：
> 换服行两端都写在行里，不会与谁重复，而且那是「人离开 A」在群里唯一的痕迹 ——
> 退服本来就不推。与他同轮到达的换服行照旧发出，所以「进服 → 落大厅 → 换走」在群里
> 只剩一行；跨轮的（大厅里等到下一轮才换走）则是大厅那条被丢 + 到台上那条进服照推，
> 同样只剩一条。
>
> 两个「配了没生效也看不出来」的位置照旧处理：写在代理上**加载期报错**（代理不参与进服
> 对账，名单是全群组口径，写了永远不生效），启动日志与 `--list-targets` 都会把它标出来
> —— 这个键哑掉时的表现正好是「本来就该安静」，不给标记就没人能发现。
...
### 9.14 P10：夜间静默（`MC_QUIET_HOURS` / `OOPZ_QUIET_HOURS`）

**要解决的问题**：用户 2026-09-26 提的「取消在夜间的 mc 播报（24:00-9:00）」。当天
09:00 验收通过（`09-26 02:00:00 MC 定时播报：夜间静默时段（0-9 点），本次跳过`，
09:00 那班照常发），随后按同一套把 oopz 的两个自动推送也加上了。

**六个设计决策**（改之前先读）：

1. **静默只掐「发消息」，不掐「取数」。** 探测照常跑、基线照常推进。这条错了的后果
   **当晚完全无声**，只在第二天 09:00 炸出来 —— 把一整晚进过服（进过频道）的人当成
   「刚进服」（「刚进频道」）一次性补报几十条。所以两边都有一条守它的断言（见
   `tools/mc_check.py --quiet-test`）。MC 那边连「静默开始前攒在限流窗口里、还没发出去的
   那几条」也一并丢掉：补发一条「三小时前有人进服」正是静默要避免的事。
2. **半开区间 `[start, end)`。** 写 `0-9` 时 `09:00` 整**不算**静默 —— 那正是「夜里
   过去了」的第一时刻，那一班要照常发。写成闭区间就得靠 `>= end` 之类的特判来救，
   而那种特判迟早被下一处调用点漏掉。
3. **陷阱之外的故障通知不受管辖。** MC 的掉线/恢复提醒刻意**不在**静默范围里：那是故障
   通知，夜里服务器真挂了，早上才发现比吵一下贵得多。所以 `_tick_audience` 的 quiet
   分支只过滤 `PlayerEvent`，`StatusEvent` 照旧（且仍然当轮强制 flush）。
4. **配错降级、不抛。** `MC_QUIET_HOURS=0-9-5` 这种手滑**不能**让机器人起不来（连着查询、
   白名单一起没），只降级成「不静默」+ 启动日志一条 ERROR。`parse_quiet_hours` 那层
   则相反 —— **格式错就抛**，因为「没配」（返回 None）和「配错」的处置完全相反。
5. **每个插件一个键、默认都是 `0-9`。** 不用一个全局键：MC 与 oopz 的推送节奏、受众
   都不一样，将来想只关一边（比如 oopz 半夜留着）不用改代码。代码默认 `0-9` 是刻意的
   —— 需求本身就是「半夜别响」，默认关等于没做；不要静默就显式留空。
6. **`@oopz` 查询 / `@mc` 查询不在静默范围里。** 那是人主动问的，跟「半夜别吵人」是
   两件事。同理 oopz 的「无人在线则跳过」照旧 —— 它本来就不发。

**共享实现**：纯逻辑（`parse_quiet_hours` / `in_quiet_hours`）在 `_shared/schedule.py`
（**不许 import nonebot**，`tools/mc_check.py` 在 `nonebot.init()` 之前就 import 它）；
读环境变量 + 启动/跃迁日志在 `_shared/quiet.py` 的 `QuietWindow`（要用 logger，所以
单独一个文件）。两个插件各建一个实例，语义由共享模块保证一致。

> **补记（2026-09-26，oopz 那条 02:00 的消息）**：加静默的动因之一，是加 MC 静默那晚
> 看到 oopz 的整点播报照推（「定时播报已推送到 1 个群」）—— 它只在「接口说有人在线」
> 时才推，所以半夜那条多半是**幽灵成员**触发的（见 5.5）。**静默不修幽灵**，它只是
> 把这条消息挪出夜间窗口；接口层的账还是 5.5 那个结论：REST 侧没有可信的判据。
