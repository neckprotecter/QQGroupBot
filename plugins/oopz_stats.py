...
# 例：OOPZ_TARGET_AREAS=01K3JDWYZTEXB18B2TPY4JMT3M
_TARGET_AREAS = [
    s.strip()
...
# 可选：只允许这些群触发查询（逗号分隔 group_openid）；留空 = 所有群都可触发
_ALLOWED_GROUPS = {
    s.strip()
    for s in os.environ.get("QQ_ALLOWED_GROUPS", "").split(",")
    if s.strip()
}
...
async def handle_stat(bot: Bot, event: MessageEvent):
    ...
    if text != "统计":
        return
    logger.info("收到@统计，来自群 {}", group_id)
