...
class PlayerEvent:
    ...
    # 统一成「这件事发生在哪台服」，文案层才能一律先打【服名】块再写行，不用为
    # SWITCH 单独分叉。
    target_id: str
    # 仅 SWITCH 有值：从哪台来。JOIN / LEAVE 恒为空 —— 跨组移动会被拆成互不相干的
    # join 和 leave，「他从哪来」在那时是**猜**出来的（他可能先退了 A、去吃了饭、
    # 再进 B），而本项目不猜。文案层因此只需在 origin_id 非空时说「从 X 换服过来」。
    origin_id: str = ""
    # 事件发生时该目标的人数，供文案里那句「当前 N 人在线」。取**到达端**本轮快照：
    # 换服时说走的那台有几个人没人关心，他在的那台才有。
    count: int = 0
...
class StatusEvent:
    ...
    why: str = ""  # "连不上了" / "应答异常"，仅 down=True 时有意义
